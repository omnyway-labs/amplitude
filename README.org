...
=amplitude= provides a set of utilities to work with amplify-js seemlessly
...
Let us take the classic =Cart= example. The following is a Graphql
...
subscribing to those mutations. While amplify-cli provides a mechanism to
generate js/typescript code, the generated code is non-performant when
having deep graphql connections. Amplitude generates the graphql
handler code dynamically during invocation time.
...
Meanwhile, we had the frontend cljs app subscribe to updates on
